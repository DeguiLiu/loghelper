...
static void PrintResult(const BenchResult& r) {
  ...
  std::printf("  %-40s  avg=%6ld ns  min=%6ld ns  max=%8ld ns  "
              "throughput=%.0f msg/s\n",
              r.name, static_cast<long>(avg_ns),
              static_cast<long>(r.min_ns), static_cast<long>(r.max_ns),
              throughput);
}
...
int main() {
  ...
  auto r2 = BenchMultiThread("Multi-thread (4T) throughput",
                              kMultiThreads, kMultiPerThread);
  PrintResult(r2);
  ...
  auto r4 = BenchCompileTimeFilter("Compile-time filtered (noop)",
                                    kSingleCount);
  PrintResult(r4);

  // 5. Runtime filter (formatted but dropped by sink)
  auto r5 = BenchRuntimeFilter("Runtime filtered (TRACE, sink=OFF)",
                                kSingleCount);
  PrintResult(r5);
